...
| 功能 | 实现方案 | 数据结构 | 验收标准 |
| :--- | :--- | :--- | :--- |
| 短期记忆 | 滑动窗口列表，保留最近20条对话 | `List[Dict]` | 窗口内对话完整可回溯 |
| 长期记忆 | SQLite 持久化存储 | `memory.db` 表结构：`id, type, content, timestamp, importance` | 重启服务后记忆可恢复 |
| 用户偏好 | 存储用户姓名/喜好/习惯 | 键值对存储 | “我叫XXX”后续对话能正确称呼 |
| 情感记忆 | 记录对话中检测到的情绪事件 | 情绪标签 + 时间戳 | 可查询“上次开心是什么时候” |
| 摘要压缩 | LLM将长对话压缩为摘要 | 每50条对话触发一次摘要 | 保留关键信息，节省Token |
| 记忆检索 | SQL `LIKE` + 全文搜索 | 关键词匹配 | “我之前说过什么”能返回相关记录 |
...
本项目为个人/小型团队开发，各角色分工如下：

| 角色 | 人数 | 职责 |
| :--- | :--- | :--- |
| 项目负责人 | 1 | 项目整体规划、技术决策、进度协调 |
| Unity 前端开发 | 1 | 3D渲染、动画、UI、桌面集成、通信模块 |
| Python AI 开发 | 1 | LLM接入、Agent推理、工具集、记忆系统 |
| C++ 底层开发 | 1 | Win32 API封装、窗口管理、系统交互 |
| 美术设计 | 1 | 3D模型、贴图、动画、UI素材制作 |
| 测试与文档 | 1 | 功能测试、Bug反馈、用户手册编写 |
...
| 术语 | 全称 | 说明 |
| :--- | :--- | :--- |
| URP | Universal Render Pipeline | Unity通用渲染管线 |
| Agent | AI Agent（智能体） | 具备自主推理和工具调用能力的AI系统 |
| LLM | Large Language Model | 大语言模型 |
| FSM | Finite State Machine | 有限状态机 |
| TTS | Text-to-Speech | 文本转语音 |
| OCR | Optical Character Recognition | 光学字符识别 |
| VLM | Vision Language Model | 视觉语言模型（多模态） |
| ReAct | Reason + Act | LangChain推理行动框架 |
| UGS | Unity Gaming Services | Unity云服务套件 |