...
> **创新设计**：本项目记忆系统采用 **"文件即记忆，数据库即索引"** 的全新理念，详细设计参见 [07-记忆系统创新设计.md](07-记忆系统创新设计.md)。

**（1）核心理念**

打破传统"记忆即数据库记录"的固有模式，实现人类可读性与机器检索效率的统一：

- **记忆本体** 以人类可读的 **Markdown 文件** 持久化存储，用户可随时查看、编辑或备份，彻底消除 AI 记忆的"黑盒"属性
- **数据库（SQLite）** 仅作为高性能检索后端，为文件内容建立多维度索引，不承担记忆内容的存储职责
- 即使数据库完全损坏，原始 Markdown 记忆文件依然完好无损，重建索引即可恢复全部记忆

**（2）五级记忆分层体系**

记忆根据时间尺度与重要性划分为五个层级，形成从"易失"到"永恒"的完整光谱：
...
> **创新点**：超短期记忆让宠物具备"察言观色"能力，能感知用户情绪波动并及时调整回应策略，使交互更具"人情味"。

**（3）文件目录结构**

记忆文件按层级分类存储，路径为 `data/memory/`：
...
数据库为 Markdown 文件建立两级索引，实现粗粒度与细粒度的协同检索：

- **文件级索引（`files` 表）**：记录每个记忆文件的 `path` / `category` / `mtime` / `hash`，通过 `mtime` + `hash` 双字段实现**增量索引**——仅当文件实际变化时才重新索引
- **内容块级索引（`chunks` 表）**：将每个 Markdown 文件切分为细粒度片段，记录 `start_line` / `end_line` / `text` / `embedding`，支持精准定位到具体段落
...
系统支持三种互补的检索路径，可按优先级依次尝试或并行执行，由 AI 自主选择最匹配结果：

1. **结构化查询**：检索 `facts` 知识图谱表，精准回答"用户叫什么名字？"等事实性问题，速度最快、零误差
2. **关键词搜索**：利用 SQLite FTS5 扩展，支持中文分词与模糊匹配，适合"之前聊过关于旅游的什么内容？"等场景
3. **语义搜索**：通过 `embedding` 字段进行向量相似度计算，处理"那个让我印象很深的事情是什么来着？"等模糊联想式问题
4. **降级策略**：以上均为空时，自动降级到 Markdown 源文件全文扫描（最慢但最全）

**（6）AI 自主记忆管理权**

记忆的管理权不再由开发者预设算法掌控，而是由 AI 根据上下文自主决策：

- **写入自主权**：AI 被赋予 `manage_memory` 工具，自主判断何时写入记忆（用户表达偏好、完成任务、会话结束、检测到长期兴趣变化等），以结构化参数指定写入内容与目标层级
- **索引维护权**：系统空闲时段自动执行摘要压缩、向量更新、事实抽取、冲突检测等后台维护任务
- **查询策略自主权**：AI 根据问题性质自主选择最优检索策略（事实性→结构化查询，开放性回忆→语义搜索，关键词明确→FTS5检索）

**（7）功能与验收标准**

| 功能 | 实现方案 | 数据结构 | 验收标准 |
| :--- | :--- | :--- | :--- |
| 五级分层管理 | `permanent` / `long_term` / `working` / `short_term` / `ultra_short` 五类目录 | Markdown 文件 + SQLite 索引 | 各层级记忆独立管理，互不干扰 |
| 文件级索引 | `files` 表追踪文件元信息，`mtime` + `hash` 增量索引 | SQLite 表 | 仅变更文件被重新索引，CPU 占用低 |
| 内容块索引 | `chunks` 表切分 Markdown 文件为细粒度片段 | SQLite 表 + embedding 向量 | 可精准定位到具体段落检索 |
| AI 自主写入 | `manage_memory` 工具，AI 自主判断写入时机与层级 | 结构化 JSON 参数 | "我叫XXX"后续对话能正确称呼 |
| 摘要压缩 | 每天凌晨 12:00 a.m. 自动触发，将 `short_term` 压缩至 `long_term`；用户仍在使用则推迟至停止使用 1 分钟后执行 | LLM 生成摘要，写入 Markdown | 压缩后当天短期记忆已归档，长期记忆可恢复 |
| 三路检索 | 结构化查询 + FTS5 全文检索 + 向量语义搜索 | `facts` 表 + FTS5 索引 + embedding | "我之前说过什么"能返回相关记录 |
| 冲突检测 | 新记忆与既有永久记忆冲突时标记并请求用户确认 | 冲突标记 + 用户审核流程 | 永久记忆不会被 AI 自动覆盖 |
| 容灾恢复 | 数据库损坏后，基于原始 Markdown 文件重建索引 | 文件系统为单一真实源 | 数据库损坏后记忆可完全恢复 |
